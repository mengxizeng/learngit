	...
	for (int i = 0; i < GRID0; i++){
		for (int j = 0; j < GRID0; j++){
			for (int k = 0; k < SPAN; k++){
				...
				int seq_maxscore = std::max_element(&data[5], &data[5] + 11) -&data[5];
				...
				float w = exp(data[2]) *anchors[k][0] /608;
				float h = exp(data[3]) *anchors[k][1] /608;
				x = (x +j)/GRID0 -w/2;
				y = (y +i)/GRID0 -h/2;
				boxes[num*4 +0] = x;
				boxes[num*4 +1] = y;
...
int process(float output0[1][GRID0][GRID0][SPAN][LISTSIZE], float output1[1][GRID1][GRID1][SPAN][LISTSIZE], float output2[1][GRID2][GRID2][SPAN][LISTSIZE],
			float* out_pos, float* out_prop, int* out_label)
{
	
	int anchors0[SPAN][2] = {{116,90}, {156,198}, {373,326}};
	int anchors1[SPAN][2] = {{30,61},  {62,45},  {59,119}};
	int anchors2[SPAN][2] = {{10,13},  {16,30},  {33,23}};
	...
	int num_candida2 = 0;
	process_feats2(output2, anchors2, OBJ_THRESH, &boxes[4 *num_candida1], &box_class[num_candida1], &box_score[num_candida1], &num_candida2);
	int num_candida = num_candida0 +num_candida1 +num_candida2 ;
...
int yolov3_post_process(float* input0, float* input1, float* input2, float* out_pos, float* out_prop, int* out_label)
{
	...
	float(&output2)[1][GRID2][GRID2][SPAN][LISTSIZE] = *((float(*)[1][GRID2][GRID2][SPAN][LISTSIZE])(buffer + LISTSIZE *SPAN *GRID1 *GRID1));
	...
	for (int i = 0; i < SPAN; i++){
		for (int j = 0; j < LISTSIZE; j++){
			...
			int bias2 = i*LISTSIZE*GRID2*GRID2+ j*GRID2*GRID2;
			for (int k = 0; k < GRID2; k++){
				int bias_k = bias2 + k*GRID2;
				for (int l = 0; l < GRID2; l++){
					output2[0][k][l][i][j] = input2[bias_k+l];
				}
			}
	...
	int object_num = process(output0, output1, output2, out_pos, out_prop, out_label);
	return object_num;
}
